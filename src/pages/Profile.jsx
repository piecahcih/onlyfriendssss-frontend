...
const Profile = ({ onRequestFriend = () => alert("Friend Request Sent!") }) => {
  // 1. ดึงข้อมูลจาก Store (แยก selector เพื่อประสิทธิภาพ)
  const storeUser = useUserStore((state) => state.user);
  const setStoreUser = useUserStore((state) => state.setUser);

  // 2. ใช้ profileData แทนชื่อ user เดิม เพื่อไม่ให้ซ้ำกับตัวแปรจาก Store
  ...
  const [editForm, setEditForm] = useState(profileData); // ใช้ profileData ตั้งต้น
  ...
  const fetchUserProfile = async () => {
    try {
      ...
      setProfileData(data); // อัปเดต State ในหน้านี้
      setEditForm(data); // อัปเดตฟอร์ม
      setStoreUser(data); // อัปเดต Store ส่วนกลาง (Zustand)
    } catch (error) {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      setProfileData(editForm); // อัปเดตหน้าจอ
      setStoreUser(editForm); // อัปเดต Store ส่วนกลาง
          ...
          <>
            ...
            >
              ...
              <div className="flex flex-col gap-3 items-start text-[18px]">
                <button onClick={hdlLogout} className="font-medium">
                  Log out
                </button>
                <button
                  onClick={"use function delete"}
          ...
          <div className="flex-1 flex flex-col">
            ...
            <div className="bg-primary w-full rounded-[30px] py-4 flex justify-around text-white shadow-lg">
              <div className="flex flex-col items-center border-r border-white/30 flex-1">
                <span className="text-lg bai-jamjuree-bold">{user?.rating ?? 0}</span>
                <span className="text-[10px] bai-jamjuree-medium opacity-90">Rating</span>
              </div>
              <div className="flex flex-col items-center border-r border-white/30 flex-1">
                <span className="text-lg bai-jamjuree-bold">{user?.events ?? 0}</span>
              ...
              <NavLink to="/friendlist" className="flex flex-col items-center flex-1">
                <span className="text-lg bai-jamjuree-bold">{user?.friends ?? 0}</span>
